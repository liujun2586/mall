...
<script type="text/javascript">
    ...
    //点击分类节点，加载该分类的属性列表（远程载入HTML的形式）
    function clickCatalog(event, treeId, treeNode) {
        $("#propertyList").load("/skuProperties/list?catalogId="+treeNode.id);
    }
    ...
    function showPropertyValue(id) {
        ...
        $("#propertyValueForm").load("/skuPropertyValue?propertyId="+id)
    }

    //刷新属性列表
    function refreshPropertyList() {
        var nodes = treeObj.getSelectedNodes();
        $("#propertyList").load("/skuProperties/list?catalogId="+nodes[0].id);
    }
    ...
    function savePropertyValue() {
        var propertyId = parseInt($("input[name='propertyId']").val());
        ...
        for(var i=0;i<$divArray.length;i++){
            var div = $divArray[i];
            //取出div中的属性值id
            var id = parseInt($(div).children("input[name='id']").val());
            //取出div中的属性值
            var value = $(div).children("input[name='value']").val();
            propertyValueArray.push({"id": id == 0 ? undefined : id,"value":value,"propertyId": propertyId == 0 ? undefined : propertyId})
        }
        console.info(JSON.stringify(propertyValueArray))
        //3、用ajax提交json数据
        $.ajax({
            url: "/skuPropertyValue",
    ...
    function deletePropertyValue(obj) {
        ...
        if(id){
            //如果id不为0，那就删除该属性值
            $.get("/skuPropertyValue/"+id,function (data) {
                if(data.success){
                    $.messager.popup("删除成功！");
                }else{
    ...
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="/skuProperties" method="post" id="propertyForm">
