...
<script type="text/javascript">
    ...
    function clickCatalog(event, treeId, treeNode) {
        $("#propertyList").load("/properties/list?catalogId="+treeNode.id);
    }
    ...
    function showPropertyValue(id) {
        ...
        $("#propertyValueForm").load("/propertyValue?propertyId="+id)
    }

    //刷新属性列表
    function refreshPropertyList() {
        var nodes = treeObj.getSelectedNodes();
        $("#propertyList").load("/properties/list?catalogId="+nodes[0].id);
    }
    ...
    function deletePropertyValue(obj) {
        ...
        if(id){
            //如果id不为0，那就删除该属性值
            $.get("/propertyValue/"+id,function (data) {
                if(data.success){
                    $.messager.popup("删除成功！");
                }else{
    ...
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="/properties" method="post" id="propertyForm">
