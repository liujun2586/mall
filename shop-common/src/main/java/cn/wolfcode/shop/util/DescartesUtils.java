...
public class DescartesUtils {
	...
	public static <T> List<List<T>> descart(List<List<T>> ops) {
		if (ops == null) {
			...
			DescartWrapper<T> wrapper = new DescartWrapper<>(ops);
			return wrapper.descart();
		}
	}

	public static class DescartWrapper<T> {
		private List<List<T>> top = new LinkedList<>();
		...
		public List<List<T>> descart() {
			...
			while (fi != null) {
				// 创建一个容器临时装拷贝的元素
				List<List<T>> copyContainer = new LinkedList<>();
				// 让top中的元素自我复制size-1份（深拷贝）；
				for (T t : fi) {
					// t = C, t = D
					// top.size = 2
					for (int j = 0; j < top.size(); j++) {
						List<T> copy = new LinkedList<>(top.get(j));
						// copy = [A]
						copy.add(t);
						// copy = [A, C], copy = [A, D]
						copyContainer.add(copy);
						// copyContainer = [[A, C], [A, D]]
					}
